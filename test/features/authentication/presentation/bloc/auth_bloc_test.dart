...
void main () {
  ...
  group('Perform Login', () {
    ...
    () async {
      ...
      // act
      bloc.add(const SignInWithCredentials(password: tPassword, username: tUsername));
      await untilCalled(mockPerformLogin(password: tPassword, username: tUsername));

      //assert
      verify(mockPerformLogin(username: tUsername, password: tPassword));
    });
    
    test('Should emit Empty state when initialized', 
    () async {
    ...
    'should emit Success state when login is successful',
      build: () {
        when(mockPerformLogin.call(username: 'myUsername', password: '123')).thenAnswer(
          (_) async => const Right(Token(tokenString: 'wswsxwsc')),
        );
        return bloc;
      },
      act: (bloc) => bloc.add(const SignInWithCredentials(username: 'myUsername', password: '123')),
      expect: () => [Success()],
    );
    ...
    'should emit Error state when login fails',
      build: () {
        when(mockPerformLogin.call(username: 'myUsername', password: '123')).thenAnswer(
          (_) async => const Left(AuthenticationFailure(message:'Authentication Failed')),
        );
        return bloc;
      },
      act: (bloc) => bloc.add(const SignInWithCredentials(username: 'myUsername', password: '123')),
